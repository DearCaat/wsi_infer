...
class Processor:
    ...
    def run_patch_feature_extraction_job(
        ...
        for wsi in self.loop:    
            ...
            if not os.path.exists(coords_path):
                coords_path = os.path.join(self.job_dir, coords_dir, 'patches', f'{wsi.name}.h5')
            if not os.path.exists(coords_path):
    ...
    def run_patch_image_extraction_job(
        self, 
        coords_dir: str, 
        saveto: str | None = None,
        img_format: str = 'png',
        ...
        """
        The `run_patch_image_extraction_job` function extracts patch images from WSIs based on 
        coordinates stored in h5 files and saves them as PNG or JPG images. This is useful for 
        visualizing patches or creating datasets for training.

        Parameters:
            coords_dir (str): 
                Path to the directory containing patch coordinates (h5 files), which are used to 
                locate patches for image extraction.
            saveto (str, optional): 
                Directory where the extracted patch images will be saved. Each slide will have 
                its own subdirectory under this path. Structure: saveto/slide_name/patch_x_y.{format}
                If not provided, a directory name will be generated automatically. Defaults to None.
            img_format (str, optional): 
                Image format for saving patches. Can be 'png' or 'jpg'. Defaults to 'png'.
            img_size (int, optional): 
                Target size for the extracted patches. If None, uses the patch_size from coords. 
                Defaults to None.

        Returns:
            str: The absolute path to where the patch images are saved.

        Example
        -------
        Extract patch images from coordinates:

        >>> processor.run_patch_image_extraction_job(
        ...     coords_dir="output/patch_coords/",
        ...     saveto="output/patch_images/",
        ...     img_format="png"
        ... )
        """
        if saveto is None:
            if save_as_h5:
                saveto = os.path.join(coords_dir, 'images_h5')
            else:
                saveto = os.path.join(coords_dir, 'images')

        os.makedirs(os.path.join(self.job_dir, saveto), exist_ok=True)

        sig = signature(self.run_patch_image_extraction_job)
        local_attrs = {k: v for k, v in locals().items() if k in sig.parameters}
        self.save_config(
            saveto=os.path.join(self.job_dir, coords_dir, '_config_imgs.json'),
            local_attrs=local_attrs,
            ignore = ['loop', 'valid_slides', 'wsis']
        )

        # Determine log file path based on save format
        if save_as_h5:
            log_fp = os.path.join(self.job_dir, coords_dir, '_logs_imgs_h5.txt')
        else:
            log_fp = os.path.join(self.job_dir, coords_dir, '_logs_imgs.txt')
        
        self.loop = tqdm(self.wsis, desc=f'Extracting patch images from coords in {coords_dir}', total = len(self.wsis))
        for wsi in self.loop:    
            # Determine file paths and lock based on save format
            if save_as_h5:
                # H5 mode: single file per slide
                wsi_output_path = os.path.join(self.job_dir, saveto, f'{wsi.name}.h5')
                lock_file = wsi_output_path
            else:
                # Individual files mode: directory per slide
                wsi_output_path = os.path.join(self.job_dir, saveto, wsi.name)
                lock_file = os.path.join(wsi_output_path, '.lock')
            ...
            # Check if slide is already fully processed by checking the final output file
            # This is the most reliable way to determine if processing is complete
            if save_as_h5:
                # For H5 mode: check if the final .h5 file exists and is not locked
                # Note: If there's an incorrect .h5 folder structure, use fix_h5_structure.py script to fix it first
                if os.path.exists(wsi_output_path) and os.path.isfile(wsi_output_path):
                    self.loop.set_postfix_str(f'Images already extracted for {wsi.name}. Skipping...')
                    continue
                    # Verify the file is valid by checking if it can be opened and has required keys
                    # file_valid = False
                    # try:
                    #     import h5py
                    #     with h5py.File(wsi_output_path, 'r') as f:
                    #         # Check if required datasets exist
                    #         if 'images' in f and 'coords' in f:
                    #             file_valid = True
                    # except Exception:
                    #     # File exists but is corrupted or cannot be opened
                    #     file_valid = False
                    
                    # if file_valid:
                    #     # File is valid and complete, skip processing
                    #     self.loop.set_postfix_str(f'Images already extracted for {wsi.name}. Skipping...')
                    #     continue
                    # else:
                    #     # File exists but is invalid or missing required keys, delete it and reprocess
                    #     try:
                    #         os.remove(wsi_output_path)
                    #         self.loop.set_postfix_str(f'Invalid h5 file found for {wsi.name}. Deleting and reprocessing...')
                    #     except Exception as e:
                    #         self.loop.set_postfix_str(f'Failed to delete invalid h5 file for {wsi.name}: {e}. Will attempt to overwrite...')
            else:
                # For individual files mode: check if directory exists and is not locked
                if os.path.exists(wsi_output_path) and not is_locked(lock_file, stale_timeout_hours=1.):
                    # Check if directory has any image files (basic check)
                    import glob
                    image_files = glob.glob(os.path.join(wsi_output_path, f'*.{img_format}'))
                    if len(image_files) > 0:
                        self.loop.set_postfix_str(f'Images already extracted for {wsi.name}. Skipping...')
                        continue

            # Check if another process has claimed this slide
            # is_locked will automatically remove stale locks (older than specified hours)
            if is_locked(lock_file, stale_timeout_hours=1.):
                self.loop.set_postfix_str(f'{wsi.name} is locked. Skipping...')
                continue

            try:
                self.loop.set_postfix_str(f'Extracting images from {wsi.name}{wsi.ext}')
                
                # Create output directory/file based on mode
                if save_as_h5:
                    final_output_dir = os.path.join(self.job_dir, saveto)
                    os.makedirs(final_output_dir, exist_ok=True)
                    # Use temporary directory for atomic write
                    # Save to temp directory first, then move h5 file to final location after completion
                    temp_output_dir = os.path.join(final_output_dir, f'.tmp_{wsi.name}')
                    temp_h5_path = os.path.join(temp_output_dir, f'{wsi.name}_patches.h5')
                    
                    # Clean up any leftover temporary directory from previous failed attempts
                ...
                update_log(log_fp, f'{wsi.name}{wsi.ext}', 'LOCKED. Extracting images...')
                
                # Extract patch images to temporary directory (for h5 mode) or directly (for individual files mode)
                if save_as_h5:
                    # Save to temporary directory first
                    # extract_patch_images will create {wsi.name}_patches.h5 in the temp directory
                    wsi.extract_patch_images(
                        coords_path=coords_path,
                        save_images=temp_output_dir,
                        img_format=img_format,
                        img_size=img_size,
                        batch_size=batch_size,
                        save_as_h5=save_as_h5,
                    )
                    # Atomic move: only move h5 file to final location after successful completion
                    # This ensures that if process crashes, the incomplete file won't be in final location
                    if os.path.exists(temp_h5_path):
                        shutil.move(temp_h5_path, wsi_output_path)
                        ...
                        raise FileNotFoundError(f"Expected h5 file not found in temp directory: {temp_h5_path}")
                else:
                    # For individual files mode, extract directly (no atomic move needed for directories)
                    wsi.extract_patch_images(
                        coords_path=coords_path,
                        save_images=wsi_output_path,
                        img_format=img_format,
                        img_size=img_size,
                        batch_size=batch_size,
                        save_as_h5=save_as_h5,
                ...
                # Log successful completion ONLY after all patches are successfully saved and moved
                # This ensures that if extraction fails, no success log is recorded
                # The h5 file is guaranteed to be complete and in final location at this point
                remove_lock(lock_file)
                update_log(log_fp, f'{wsi.name}{wsi.ext}', 'Images extracted.')
            except Exception as e:
                # Remove lock on error
                remove_lock(lock_file)
                
                # Clean up temporary directory if it exists (for h5 mode)
                # Note: We don't need to remove final output file since we use temp directory
                if save_as_h5 and temp_output_dir and os.path.exists(temp_output_dir):
                    try:
                        # Remove entire temp directory (including any files inside)
                        shutil.rmtree(temp_output_dir)
                    except Exception:
                        pass
