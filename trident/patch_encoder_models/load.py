...
def encoder_factory(model_name: str, **kwargs):
    ...
    elif model_name == 'hipt_4k':
        enc =  HIPT4kInferenceEncoder
    elif model_name == 'hipt':
        enc = HIPTInferenceEncoder
    else:
...
class HIPT4kInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        self.enc_name = 'hipt_4k'
        weights_path = self.weights_path

        from .model_zoo.hipt.hipt import HIPT_4K

        model = HIPT_4K(model256_path = os.path.join(weights_path,'vit256_small_dino.pth'),
                        model4k_path = os.path.join(weights_path,'vit4k_xs_dino.pth'))


        mean, std = (0.5, 0.5, 0.5), (0.5, 0.5, 0.5)
        eval_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean = mean, std = std)])

        precision = torch.float32

        return model, eval_transform, precision

class HIPTInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        from .model_zoo.hipt.hipt import HIPT

        model = HIPT(model256_path = os.path.join(weights_path,'vit256_small_dino.pth'))
...
class UNIInferenceEncoder(BasePatchEncoder):
    ...
    def _build(
        ...
        from trident.patch_encoder_models.model_zoo.attention import Attention
        
        self.enc_name = 'uni_v1'
        UNI_CKPT_PATH=os.environ.get('UNI_CKPT_PATH')

        if UNI_CKPT_PATH:
            weights_path = UNI_CKPT_PATH
        else:
            weights_path = self._get_weights_path()
        ...
        if weights_path:
            try:
                ...
                # Try to use attn_layer if supported (timm >= 0.9.12)
                try:
                    model = timm.create_model(
...
class UNIv2InferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        if weights_path:
            try:
                try:
                    model = timm.create_model(model_name='vit_giant_patch14_224', pretrained=False, attn_layer=Attention, **timm_kwargs)
                except TypeError:
                    # attn_layer not supported
            ...
            try:
                try:
                    model = timm.create_model("hf-hub:MahmoodLab/UNI2-h", pretrained=True, attn_layer=Attention, **timm_kwargs)
                except TypeError:
                    # attn_layer not supported
