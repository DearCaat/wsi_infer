...
# Local Dependencies
# from vision_transformer import vision_transformer as vits
# from vision_transformer4k import vision_transformer4k as vits4k
...
class HIPT(torch.nn.Module):
	...
	def forward(self, x):
		...
		for mini_bs in range(0, batch_256.shape[0], 256):                       # 3. B may be too large for ViT-256. We further take minibatches of 256.
			minibatch_256 = batch_256[mini_bs:mini_bs+256].to(_device, non_blocking=True)
		...
		# features_cls256 = features_cls256.reshape(-1, 256, 384)
...
class HIPT_4K(torch.nn.Module):
	...
	def forward(self, x):
		...
		features_cls256 = torch.vstack(features_cls256)                        # 3. [B x 384], where 384 == dim of ViT-256 [ClS] token.
		features_cls256 = features_cls256.reshape(-1, w_256, h_256, 384).transpose(1,2).transpose(1,3) 
		features_cls256 = features_cls256.to(_device, non_blocking=True)  # 4. [1 x 384 x w_256 x h_256]
